...
export type PersonaType = 'normal' | 'donald' | 'vladimir' | 'sanae' | 'jack' | 'amari' | 'sato' | 'shinchan' | 'toru' | 'inamori' | 'ohara_miko' | 'vega' | 'shibai' | 'yoichi' | 'ryuoh';
export type UnionPersonaType = 'kiyomi' | 'kasshiro' | 'baigaeshi' | 'hiroyuki' | 'aibo' | 'gonda' | 'rational' | 'wakui' | 'zenkouwan' | 'masako' | 'kaishu' | 'chunli' | 'shokatsu' | 'soren';
...
export interface SimulationConfig {
  ...
  autoRaiseEnabled?: boolean; // 定期昇給の自動適用フラグ (デフォルト: false)
  convertCurrentReempToExtension?: boolean; // 現・再雇用者を延長社員へ移行するか (default: false)
  extensionImplementationDate?: string; // 定年延長実施日 (YYYY-MM-DD)
  // ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
...
export interface RaisePlanYear {
    ...
    detailed: {
        ...
        reemp_shokutaku: number; // Added for Re-employment (Shokutaku)
    };
    yearlyRaise: number;
    raiseRate: number;
}
