...
export const IndividualDetailCView: React.FC<{ resultC: SimulationResult }> = ({ resultC }) => {
    const [selectedYear, setSelectedYear] = useState<number>(
        (resultC && resultC.summary && resultC.summary.length > 0) ? resultC.summary[0].year : 2026
    );
    const [searchTerm, setSearchTerm] = useState("");

    useEffect(() => {
        if (resultC && resultC.summary && resultC.summary.length > 0) {
            if (!resultC.summary.find(r => r.year === selectedYear)) {
                setSelectedYear(resultC.summary[0].year);
            }
        }
    }, [resultC]);

    const employees: IndividualResult[] = useMemo(() => {
        if (!resultC || !resultC.individuals) return [];
        return resultC.individuals
            .filter(i => i.year === selectedYear && (i.name.includes(searchTerm) || String(i.id).includes(searchTerm)))
            .sort((a, b) => (b.total || 0) - (a.total || 0));
    }, [resultC, selectedYear, searchTerm]);
            ...
            <div className="p-4 border-b bg-gray-50 dark:bg-gray-900 dark:border-gray-700">
                <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center gap-4">
                        ...
                        >
                            {resultC && resultC.summary && resultC.summary.length > 0 ? (
                                resultC.summary.map(r => <option key={r.year} value={r.year}>{r.year}年度</option>)
                            ) : (
                    ...
                    <TableToolbar 
                        title={`${selectedYear}年度 個人別明細[C案] (${employees.length}名)`} 
                        data={exportData} 
                        filename={`individual_detail_C_${selectedYear}`} 
                    />
                </div>
            </div>
