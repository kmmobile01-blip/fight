...
export const createConfigA = (): SimulationConfig => {
    ...
    if(settings["正社員(延長)"]) {
        ...
        // Default A: Fixed Amount 224,020, useCurrentIfLower: true (Default as requested)
        settings["正社員(延長)"].calculationMethod = 'fixed';
        settings["正社員(延長)"].fixedSalary = 224020;
        settings["正社員(延長)"].useCurrentIfLower = true;
    ...
    return { 
        label: 'パターンA', 
        extendedRetirementAge: 65, // Default set to 65 years old
        reemploymentAge: 65, 
        partTimeAgeLimit: 73, 
        cutRate: 1.0, 
        ...
        convertCurrentReempToExtension: false, // Default: Keep current Re-emp as Re-emp
